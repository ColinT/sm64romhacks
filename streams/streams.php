<?php				
	...
	function renderStreams($streams) {
		...
		if (sizeof($streams) == 0) {print("<div></div><div>Currently Nobody is streaming!</div>");}
		foreach($streams as $stream) {
			if(str_contains(strtolower($stream->title), "romhack") || str_contains(strtolower($stream->title), "rom hack") || validateTags($stream->tags)) {
